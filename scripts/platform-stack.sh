...
ensure_runtime_dirs() {
  mkdir -p "$ROOT_DIR/platform-projects"
}
...
validate_env() {
  ...
  local jwt_secret
  jwt_secret=$(extract_env_val "JWT_SECRET")
  ...
  anon_key=$(extract_env_val "SUPABASE_ANON_KEY")
  service_key=$(extract_env_val "SUPABASE_SERVICE_KEY")
  if [[ -z "$anon_key" || -z "$service_key" ]]; then
    errors+=("SUPABASE_ANON_KEY and SUPABASE_SERVICE_KEY must be populated")
  fi

...
const buildListObjectsPayload = ({ path, options }: ListObjectsOptions) => {
  ...
  if (sanitizedPath.length === 0) {
    prefix = ''
    search = ''
  } else if (sanitizedPath.endsWith('/')) {
    prefix = sanitizedPath
    search = ''
  } else {
    const lastSeparator = sanitizedPath.lastIndexOf('/')
    if (lastSeparator >= 0) {
      prefix = sanitizedPath.slice(0, lastSeparator + 1)
      search = sanitizedPath.slice(lastSeparator + 1)
    } else {
      prefix = ''
      search = sanitizedPath
    }
  }

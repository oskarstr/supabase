...
- Reconstruct the orchestration dataflow end-to-end (Studio → platform-api → runtime-agent → Supabase runtime stack/Docker) and confirm each hop’s contracts and assumptions.
...
- Runtime agent drives provisioning with deterministic port allocation and host-visible runtime dirs (`b5ca522836`, `12c266ec81`).
- **Open tasks**
- Document the full provisioning contract (status transitions, response payload fields, failure semantics) so Studio/tests share a single spec. *(Draft captured in `.agent/docs/provisioning-contract.md`; end-to-end tests still pending.)*
  ...
  - Capture structured runtime-agent output/error metadata for operators and tests.
...
- Replace the temporary host binary dependency with fully in-process orchestration. Runtime agent already executes the mirrored Supabase packages in-process, captures stdout/stderr/duration, and returns that metadata via the control plane. Next step is to mount it in compose with docker socket + host network and stream results into persistence.
...
- **Studio alignment**
  ...
  - When `local` is selected, clamp provider/region to LOCAL defaults, hide remote-only pricing/instance UI, and surface runtime/Docker prerequisites.
...
- **2025-10-16 04:20 UTC · codex** – Removed the legacy `supabase` binary from platform-api images/compose, enforced runtime-agent bearer auth, and made orchestration flows require the agent or explicit override hooks.
...
- **2025-10-15 06:55 UTC · 12c266ec81** – Provisioner now routes through the runtime agent on the shared compose network; runtime dirs moved under `platform-projects/`; added `LOCAL` provider enum.
- **2025-10-15 07:28 UTC · d4e4dcfc16** – Added local runtime health polling, pause/resume endpoints, and persisted service exclusions for agent-managed project stacks.
- **2025-10-15 05:33 UTC · b5ca522836** – (Historical) Initially bundled the `supabase` binary + Docker client into the platform-api image, added port allocation + config scaffolding, and exposed the Studio deployment-target toggle.
